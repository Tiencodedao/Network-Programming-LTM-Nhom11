...
class CaroAI:
    ...
    PATTERNS = {
        'five': 100000000,   # 5 quân liên tiếp - thắng
        'open_four': 10000000,  # 4 quân 2 đầu trống: _XXXX_ (chắc thắng)
        'four': 500000,      # 4 quân 1 đầu bị chặn: XXXX_ hoặc _XXXX
        'double_open_three': 5000000,  # 2 dãy 3 quân mở - chắc thắng trong 2 nước
        'open_three': 50000,  # 3 quân 2 đầu trống: _XXX_ (nguy hiểm)
        'broken_four': 10000, # 4 quân bị ngắt: XX_X hoặc X_XX
        'three': 1000,       # 3 quân 1 đầu bị chặn
        'open_two': 500,     # 2 quân 2 đầu trống: _XX_
        'two': 100,          # 2 quân
        'one': 10            # 1 quân
    }
    ...
    def __init__(self, symbol='O', difficulty='medium'):
        ...
        self.max_depth = {'easy': 1, 'medium': 3, 'hard': 4}[difficulty]
    ...
    def _get_smart_move(self, board, rows, cols, win_condition):
        ...
        3. Kiểm tra open four (4 quân mở) - chắc thắng
        4. Chặn open four của đối thủ
        5. Tạo double threat (2 mối đe dọa cùng lúc)
        6. Chặn double threat của đối thủ
        7. Tạo các dãy nguy hiểm
        8. Chọn vị trí tốt nhất dựa trên điểm số
        """
        ...
        # 2. Chặn đối thủ nếu có thể thắng ngay
        blocking_move = self._find_winning_move(board, rows, cols, win_condition, self.opponent_symbol)
        if blocking_move:
            return blocking_move

        # 3. Tạo open four (4 quân mở) - chắc thắng
        open_four_move = self._find_open_four_move(board, rows, cols, win_condition, self.symbol)
        if open_four_move:
            return open_four_move

        # 4. Chặn open four của đối thủ
        block_open_four = self._find_open_four_move(board, rows, cols, win_condition, self.opponent_symbol)
        if block_open_four:
            return block_open_four

        # 5. Tạo double threat (2 đường tấn công cùng lúc)
        double_threat_move = self._find_double_threat_move(board, rows, cols, win_condition, self.symbol)
        if double_threat_move:
            return double_threat_move

        # 6. Chặn double threat của đối thủ
        block_double_threat = self._find_double_threat_move(board, rows, cols, win_condition, self.opponent_symbol)
        if block_double_threat:
            return block_double_threat

        # 7. Đánh giá tất cả các nước đi có thể
        scored_moves = []
        ...
        for r, c in candidates:
            ...
            # Thêm điểm cho các threat có thể tạo ra
            threat_count = self._count_threats(board, r, c, self.symbol, win_condition, rows, cols)
            attack_score += threat_count * 10000
            ...
            # Thêm điểm cho các threat đối thủ có thể tạo ra (cần chặn)
            opponent_threats = self._count_threats(board, r, c, self.opponent_symbol, win_condition, rows, cols)
            defense_score += opponent_threats * 10000
    ...
    def _find_open_four_move(self, board, rows, cols, win_condition, symbol):
        """Tìm nước đi tạo 4 quân mở (chắc thắng) hoặc chặn đối thủ"""
        candidates = self._get_candidate_moves(board, rows, cols)
        for r, c in candidates:
            if self._get_cell(board, r, c) == "":
                self._set_cell(board, r, c, symbol)
                # Kiểm tra xem có tạo ra 4 quân mở không
                if self._has_open_four(board, r, c, symbol, win_condition, rows, cols):
                    self._set_cell(board, r, c, "")
                    return (r, c)
                self._set_cell(board, r, c, "")
        return None

    def _has_open_four(self, board, row, col, symbol, win_condition, rows, cols):
        ...
        for dr, dc in directions:
            ...
            r, c = row + dr, col + dc
            front_space = 0
            while 0 <= r < rows and 0 <= c < cols and front_space < 2:
            ...
            r, c = row - dr, col - dc
            back_space = 0
            while 0 <= r < rows and 0 <= c < cols and back_space < 2:
                if self._get_cell(board, r, c) == symbol:
                    count += 1
                elif self._get_cell(board, r, c) == "":
                    open_ends += 1
            ...
            # 4 quân mở = 4 quân liên tiếp và 2 đầu trống
            if count == 4 and open_ends == 2:
                return True

        return False

    def _find_double_threat_move(self, board, rows, cols, win_condition, symbol):
        """Tìm nước đi tạo ra 2 mối đe dọa cùng lúc (double threat) - không thể chặn được"""
        candidates = self._get_candidate_moves(board, rows, cols)
        for r, c in candidates:
            if self._get_cell(board, r, c) == "":
                self._set_cell(board, r, c, symbol)
                threat_count = self._count_threats(board, r, c, symbol, win_condition, rows, cols)
                self._set_cell(board, r, c, "")

                # Nếu tạo ra >= 2 mối đe dọa nghiêm trọng (3 quân mở hoặc 4 quân)
                if threat_count >= 2:
                    return (r, c)
        return None

    def _count_threats(self, board, row, col, symbol, win_condition, rows, cols):
        """Đếm số mối đe dọa nghiêm trọng tại vị trí này"""
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        threat_count = 0
        ...
        for dr, dc in directions:
            ...
            # Đếm các mối đe dọa nghiêm trọng:
            # - 4 quân (1 hoặc 2 đầu mở)
            # - 3 quân mở (2 đầu trống)
    ...
    def _evaluate_board_position(self, board, row, col, symbol, win_condition, rows, cols):
        ...
        Bao gồm cả broken patterns (XX_X, X_XX, etc.)
        """
        total_score = 0
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        ...
        for dr, dc in directions:
            ...
            count = 1
            open_ends = 0
            gaps = 0  # Số khoảng trống trong dãy
            ...
            while 0 <= r < rows and 0 <= c < cols and steps_forward < win_condition:
                if self._get_cell(board, r, c) == symbol:
                    count += 1
                    gaps += temp_gaps  # Thêm các gap đã gặp
                    temp_gaps = 0
                    r += dr
                    c += dc
                    steps_forward += 1
                elif self._get_cell(board, r, c) == "" and temp_gaps == 0 and steps_forward < win_condition - 1:
                    # Cho phép 1 khoảng trống trong dãy
                    temp_gaps += 1
                    r += dr
                    c += dc
                    steps_forward += 1
            ...
            while 0 <= r < rows and 0 <= c < cols and steps_backward < win_condition:
                if self._get_cell(board, r, c) == symbol:
                    ...
                    c -= dc
                    steps_backward += 1
                elif self._get_cell(board, r, c) == "" and temp_gaps == 0 and steps_backward < win_condition - 1:
                    temp_gaps += 1
                    r -= dr
                    c -= dc
                    steps_backward += 1
            ...
            if count >= win_condition:
                ...
                if gaps == 1:
                    # Broken four: XX_X hoặc X_XX
                    total_score += self.PATTERNS['broken_four']
                elif open_ends == 2:
